...
int main(int argc, char *argv[]) {
    ...
    parser.setApplicationDescription(
        QStringLiteral("Batch-convert local HTML documents to PDF using Qt WebEngine."));
    ...
    const QCommandLineOption settleOption(
        ...
        QStringLiteral("Delay after resources are ready in milliseconds (default: 1000)."),
        QStringLiteral("milliseconds"), QStringLiteral("1000"));

    parser.addOptions({inputOption, outputOption, overwriteOption, addSuffixOption,
                       allowRemoteOption, timeoutOption, settleOption});
    parser.process(app);

    const bool commandLineMode = parser.isSet(inputOption) || parser.isSet(outputOption);
    if (!commandLineMode) {
        MainWindow window;
        window.show();
        return app.exec();
    ...
    if (!parser.isSet(inputOption) || !parser.isSet(outputOption)) {
        errorStream << "Both --input and --output are required in command-line mode.\n";
        return 2;
    }
    ...
    if (!timeoutValid || timeoutSeconds < 1 || !settleValid || settleMilliseconds < 0) {
        errorStream << "Timeout values must be valid non-negative numbers.\n";
        return 2;
    }

    QString discoveryError;
    const QStringList inputFiles = FileDiscovery::findHtmlFiles(
        parser.value(inputOption), &discoveryError);
    if (!discoveryError.isEmpty()) {
        errorStream << discoveryError << '\n';
        return 2;
    }
    if (inputFiles.isEmpty()) {
        errorStream << "No .html or .htm files were found in the input directory.\n";
        return 2;
    }
    ...
    QObject::connect(&controller, &ConversionController::logMessage,
                     &app, [&outputStream](const QString &message) {
        outputStream << message << '\n';
        outputStream.flush();
    });
    QObject::connect(&controller, &ConversionController::batchFinished,
                     &app, [&app](int, int failed, int) {
