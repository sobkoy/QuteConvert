...
ConversionController::ConversionController(QObject *parent)
    ...
    connect(converter_, &HtmlToPdfConverter::loadProgress,
            this, &ConversionController::fileLoadProgress);
    connect(converter_, &HtmlToPdfConverter::completed,
            this, &ConversionController::handleConversionCompleted);
}
...
void ConversionController::start(const QStringList &inputFiles,
    ...
    for (const QString &inputPath : inputFiles) {
        QString outputPath = QFileInfo(
            FileDiscovery::outputPathFor(inputPath, outputDirectory)).absoluteFilePath();

        if (options.existingFilePolicy == ExistingFilePolicy::AddSuffix
            || reservedOutputPaths.contains(outputPath)) {
            outputPath = FileDiscovery::uniqueOutputPath(outputPath,
                                                         reservedOutputPaths);
        }
...
void ConversionController::cancel() {
    ...
    emit logMessage(QStringLiteral("Cancelling after the active operation stops..."));
...
void ConversionController::processNext() {
    ...
    if (options_.existingFilePolicy == ExistingFilePolicy::Skip
        ...
        ++skipped_;
        ++currentIndex_;
        const QString message = QStringLiteral("Skipped existing file: %1")
                                    .arg(job.outputPath);
...
void ConversionController::handleConversionCompleted(
    ...
    if (success) {
        ...
        ++failed_;
        emit logMessage(QStringLiteral("Failed %1: %2")
                            .arg(job.inputPath, errorMessage));
    }
