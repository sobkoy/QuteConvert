...
QStringList FileDiscovery::findHtmlFiles(const QString &directoryPath,
    ...
    std::sort(files.begin(), files.end(), [&collator](const QString &left,
                                                       const QString &right) {
...
QString FileDiscovery::outputPathFor(const QString &inputPath,
    ...
    return QDir(outputDirectory).filePath(inputInfo.completeBaseName()
                                          + QStringLiteral(".pdf"));
}
...
QString FileDiscovery::uniqueOutputPath(const QString &desiredPath,
    ...
    const QString normalizedDesired = QDir::cleanPath(desiredInfo.absoluteFilePath());
