...
MainWindow::MainWindow(QWidget *parent)
    ...
    inputLayout->addWidget(new QLabel(QStringLiteral("HTML directory:"), central));
    inputPathEdit_ = new QLineEdit(central);
    inputPathEdit_->setPlaceholderText(QStringLiteral("Directory containing HTML files"));
    ...
    outputLayout->addWidget(new QLabel(QStringLiteral("PDF directory:"), central));
    outputPathEdit_ = new QLineEdit(central);
    outputPathEdit_->setPlaceholderText(QStringLiteral("Directory for generated PDFs"));
    ...
    fileCountLabel_ = new QLabel(QStringLiteral("No directory selected"), central);
    ...
    auto *advancedGroup = new QGroupBox(QStringLiteral("PDF and loading options"), central);
    ...
    orientationCombo_->addItem(QStringLiteral("Landscape"), QPageLayout::Landscape);
    ...
    advancedLayout->addRow(QStringLiteral("Post-load settling delay:"), settleDelaySpin_);
    ...
    advancedLayout->addRow(QStringLiteral("If output exists:"), existingFileCombo_);
    ...
    logEdit_->setPlaceholderText(QStringLiteral("Conversion messages will appear here."));
    mainLayout->addWidget(logEdit_, 1);
    setCentralWidget(central);

    connect(inputBrowseButton_, &QPushButton::clicked,
            this, &MainWindow::chooseInputDirectory);
    ...
    connect(inputPathEdit_, &QLineEdit::editingFinished,
            this, &MainWindow::refreshFiles);
    connect(startButton_, &QPushButton::clicked,
            this, &MainWindow::startConversion);
    connect(cancelButton_, &QPushButton::clicked,
            controller_, &ConversionController::cancel);

    connect(controller_, &ConversionController::batchStarted, this, [this](int total) {
        progressBar_->setRange(0, total);
        progressBar_->setValue(0);
        setBatchRunning(true);
    });
    connect(controller_, &ConversionController::fileStarted,
            this, [this](const QString &path, int current, int total) {
        statusLabel_->setText(QStringLiteral("%1 of %2: %3")
                                  .arg(current).arg(total)
                                  .arg(QFileInfo(path).fileName()));
    });
    connect(controller_, &ConversionController::overallProgress,
            this, [this](int completed, int total) {
        progressBar_->setRange(0, total);
        progressBar_->setValue(completed);
    });
    connect(controller_, &ConversionController::logMessage,
            logEdit_, &QPlainTextEdit::appendPlainText);
    connect(controller_, &ConversionController::batchFinished,
            this, [this](int succeeded, int failed, int skipped) {
        setBatchRunning(false);
        statusLabel_->setText(QStringLiteral("Finished: %1 created, %2 failed, %3 skipped")
                                  .arg(succeeded).arg(failed).arg(skipped));
    });
}

void MainWindow::chooseInputDirectory() {
    const QString initialPath = inputPathEdit_->text().isEmpty()
...
void MainWindow::refreshFiles() {
    ...
    inputFiles_ = FileDiscovery::findHtmlFiles(inputPathEdit_->text(), &errorMessage);
    fileList_->clear();
    for (const QString &path : inputFiles_) {
        fileList_->addItem(QFileInfo(path).fileName());
    }
    fileCountLabel_->setText(errorMessage.isEmpty()
        ? QStringLiteral("%1 HTML file(s), non-recursive scan").arg(inputFiles_.size())
        : errorMessage);
...
void MainWindow::startConversion() {
    ...
    if (inputFiles_.isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("No HTML files"),
                             QStringLiteral("The selected directory contains no readable "
                                            ".html or .htm files."));
        return;
    }
    if (outputPathEdit_->text().trimmed().isEmpty()) {
        QMessageBox::warning(this, QStringLiteral("No output directory"),
...
ConversionOptions MainWindow::selectedOptions() const {
    ...
    options.pageSize = QPageSize(
        static_cast<QPageSize::PageSizeId>(pageSizeCombo_->currentData().toInt()));
